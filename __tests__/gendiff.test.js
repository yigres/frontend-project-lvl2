...
const expectedString = () => {
  ...
  const result = `{${Object.entries(JSON.parse(readJsonFile)).reduce((acc, [a, b]) => `${acc}\n${a}: ${b}`, '')}\n}`;
  return result;
};
...
test('generate difference for two json files', () => {
  expect(gendiff(path1, path2)).toBe(expectedString());
});

test('generate difference for two yaml files', () => {
  expect(gendiff(path3, path4)).toBe(expectedString());
});
